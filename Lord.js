...
module.exports = Zeeone = async (Zeeone, Ofc) => {
	try {
		...
		const alphaNumber = [`6285731621782@s.whatsapp.net`, `918156874290@s.whatsapp.net` ]
...
var waktoo = date.getHours();
		...
		switch(bulan1) {
    	    ...
    	    if(time2 < "05:00:00"){
				...
				mess = {
					wait: 'Proses Kak..',
					limit: 'Limit kamu habis Kak , Silahkan beli dengan #buylimit <nominal> atau minta gift dari user lain',
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`HanifUserbot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
